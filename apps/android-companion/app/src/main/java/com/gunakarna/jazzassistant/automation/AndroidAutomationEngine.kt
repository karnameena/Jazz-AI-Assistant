...
class AndroidAutomationEngine(private val service: AccessibilityService) {
    ...
    fun execute(action: String, args: Map<String, Any?> = emptyMap()): Map<String, Any?> = when (action) {
        ...
        "scroll_forward", "scroll_down" -> simple(actions.scrollForward(), "SCROLL_SUCCESS", "Scrolled down.")
        "scroll_backward", "scroll_up" -> simple(actions.scrollBackward(), "SCROLL_SUCCESS", "Scrolled up.")
