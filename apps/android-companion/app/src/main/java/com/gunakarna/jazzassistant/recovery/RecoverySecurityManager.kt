...
class RecoverySecurityManager(private val context: Context) {
    companion object {
        ...
        private const val TOKEN_REPAIR_REQUIRED = "token_repair_required"
    ...
    fun pairingToken(): String {
        ...
        if (!encrypted.isNullOrBlank() && !iv.isNullOrBlank()) {
            try {
                ...
                // Android Auto Backup / reinstall can restore encrypted SharedPreferences
                // without the original Android Keystore key. In that state AES/GCM throws
                // AEADBadTagException. The old token cannot be recovered, so rotate it
                // locally and require the owner to copy the new token to the relay.
                clearBrokenStoredToken()
                val replacement = createAndStoreToken()
                prefs.edit().putBoolean(TOKEN_REPAIR_REQUIRED, true).apply()
                return replacement
            }
        }
    ...
    fun pairingTokenNeedsRelayUpdate(): Boolean = prefs.getBoolean(TOKEN_REPAIR_REQUIRED, false)

    fun markPairingTokenSynchronized() {
        prefs.edit().putBoolean(TOKEN_REPAIR_REQUIRED, false).apply()
    }
    ...
    fun signedHeaders(method: String, path: String, body: String): Map<String, String> {
        ...
        val signature = hmacHex(token, canonical)
        return mapOf(
    ...
    private fun clearBrokenStoredToken() {
        prefs.edit().remove(TOKEN_CT).remove(TOKEN_IV).apply()
        try {
            val keyStore = KeyStore.getInstance("AndroidKeyStore").apply { load(null) }
            if (keyStore.containsAlias(KEY_ALIAS)) keyStore.deleteEntry(KEY_ALIAS)
        } catch (_: Exception) {
            // A new key will be generated by storageKey() below.
        }
    }
