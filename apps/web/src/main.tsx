...
import { Bell, CalendarDays, Check, ChevronDown, FileText, Globe, Laptop, Menu, MessageSquare, Mic, MoreVertical, Paperclip, Phone, Plus, Send, Settings, Smartphone, Sun, Zap } from "lucide-react";
import "./styles.css";
...
const initialMessages: Message[] = [
  ...
  { id: 2, sender: "jazz", text: "Good morning, Mama! ☀️\n\nThe weather today is looking pleasant.\n\n• Temperature: 28°C\n• Condition: Partly Cloudy\n• Humidity: 62%\n• Wind: 10 km/h\n\nIt's a pleasant day! Perfect for a walk or getting some fresh air. ☀️", time: "10:31 AM" },
    ...
    <main className="main-content">
      ...
      <div className="dashboard-grid">
        ...
        <aside className="right-panel">
          <DashboardCard icon={<Zap size={20} />} title="Quick Actions"><div className="quick-actions"><QuickAction icon={<FileText />} label="Take a Note" /><QuickAction icon={<Bell />} label="Set Reminder" /><QuickAction icon={<CalendarDays />} label="Open Calendar" /><QuickAction icon={<Globe />} label="Search Web" /></div></DashboardCard>
          ...
          <DashboardCard icon={<Bell size={20} />} title="Upcoming Reminders" action="See all"><div className="reminder-list"><Reminder icon={<Phone />} title="Call Mom" date="Today, 7:00 PM" /><Reminder icon={<CalendarDays />} title="Project Meeting" date="Tomorrow, 10:00 AM" /></div></DashboardCard>
...
function QuickAction({ icon, label }: { icon: React.ReactNode; label: string }) { return <button className="quick-action"><span>{icon}</span>{label}</button>; }
